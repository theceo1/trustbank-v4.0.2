...
export async function POST(
  ...
  try {
    ...
    const { resolution, admin_notes, winner_id } = body;

    if (!resolution || !winner_id) {
      return NextResponse.json({ error: 'Resolution and winner_id are required' }, { status: 400 });
    }
    ...
    // Verify winner is involved in trade
    if (winner_id !== dispute.trade.buyer_id && winner_id !== dispute.trade.seller_id) {
    ...
    const escrowResponse = await quidax.transferToSubAccount(process.env.QUIDAX_ESCROW_WALLET_ID!, {
      ...
      fund_uid: winner_id === dispute.trade.buyer_id ? dispute.trade.buyer_quidax_id : dispute.trade.seller_quidax_id,
      transaction_note: `P2P trade dispute resolution for trade ${dispute.trade_id}`,
      narration: 'P2P trade dispute resolution'
    });
    ...
    console.error('P2P dispute resolution error:', error);
