...
export default function P2POrderDetailsPage({ params }: { params: { id: string } }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        const response = await fetch(`/api/trades/p2p/orders/${params.id}`, {
          method: 'GET',
          headers: {
    ...
    fetchOrder();
  }, [params.id, user, profileLoading, supabase]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetch(`/api/trades/p2p/orders/${params.id}/trade`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
